...
int main(int argc, char **argv) {
  ...
  *(udp_payload_string + len - 1) =
  ...
  tcp_connection(full_path, tcp_preprob_port, server_ip,
                 true); // Pre-Probing Phase TCP Connection

  probing_phase(server_ip, server_wait_time, measurement_time, dst_port,
                src_port, payload_size, train_size); // Probing Phase

  tcp_connection(full_path, tcp_postprob_port, server_ip,
                 false); // Post-Probing Phase TCP Connection
